...
class ChatAnalyzer:
    ...
    def cluster_messages(self, df, method='lda', n_groups=5):
        try:
            if len(df) < n_groups:
                raise ValueError(f"No hay suficientes mensajes para crear {n_groups} grupos")
            ...
            if method == 'kmeans':
                ...
                model = LatentDirichletAllocation(n_components=n_groups, random_state=42)
    ...
    def save_results(self, df, keywords, directory):
        try:
            ...
            for cluster in df['cluster'].unique():
                ...
                self._save_cluster_file(df_cluster, cluster, keywords[cluster], directory)
                
            # Save summary file
            self._save_summary_file(df, keywords, directory)
    ...
    def analyze(self, file_path, method='lda'):
        try:
            ...
            directory = f"resultados_chat_{timestamp}"
            
            df_processed, keywords = self.cluster_messages(df, method)
            self.save_results(df_processed, keywords, directory)
            ...
            if file_path.startswith('temp_'):
                try:
                    ...
                    pass  # Ignoramos errores en la limpieza
    ...
    def _get_stop_words(self):
        return [
            ...
            'si', 'porque', 'muy', 'sin', 'sobre', 'mi', 'hay', 'bien',
            'cuando', 'ahora', 'esta', 'asi', 'nos', 'ni', 'ese', 'eso',
            'esto', 'etc', 'otro', 'tras'
        ]
    ...
    def _save_cluster_file(self, df_cluster, cluster_num, keywords, directory):
        try:
            ...
            with open(file_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"Período: {df_cluster['fecha'].min()} a {df_cluster['fecha'].max()}\n\n")
                ...
                for fecha in sorted(df_cluster['fecha'].unique()):
                    ...
                    for _, row in mensajes_fecha.iterrows():
                        f.write(f"{row['hora']} - {row['usuario']}: {row['mensaje_original']}\n")
        except Exception as e:
            ...
            with open(file_path, 'w', encoding='utf-8') as f:
                ...
                for cluster in sorted(df['cluster'].unique()):
                    ...
                    if not df_cluster.empty:
                        ...
                        f.write(f"  {ejemplo['usuario']}: {ejemplo['mensaje_original'][:100]}...\n")
        except Exception as e:
